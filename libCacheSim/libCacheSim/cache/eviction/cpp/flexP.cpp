...
extern "C"
{
    ...
    static void flexP_free(cache_t *cache);
    static bool flexP_get(cache_t *cache, const request_t *req);

    static cache_obj_t *flexP_find(cache_t *cache, const request_t *req,
                                   const bool update_cache);
    static cache_obj_t *flexP_insert(cache_t *cache, const request_t *req);
    static cache_obj_t *flexP_to_evict(cache_t *cache, const request_t *req);
    static void flexP_evict(cache_t *cache, const request_t *req);
    static bool flexP_remove(cache_t *cache, const obj_id_t obj_id);
    static inline int64_t flexP_get_occupied_byte(const cache_t *cache);
    static inline int64_t flexP_get_n_obj(const cache_t *cache);
    static inline bool flexP_can_insert(cache_t *cache, const request_t *req);
    static void flexP_update(cache_t *cache);
    static void flexP_print(cache_t *cache);
    static void flexP_aging(cache_t *cache);
    static void flexP_parse_params(cache_t *cache,
                                   const char *cache_specific_params);

    cache_t *flexP_init(const common_cache_params_t ccache_params,
                        const char *cache_specific_params)
    {
        cache_t *cache = cache_struct_init("flexP", ccache_params, cache_specific_params);
        cache->eviction_params = reinterpret_cast<void *>(new eviction::flexP_params_t);

        cache->cache_init = flexP_init;
        cache->cache_free = flexP_free;
        cache->get = flexP_get;
        cache->find = flexP_find;
        cache->insert = flexP_insert;
        cache->evict = flexP_evict;
        cache->to_evict = flexP_to_evict;
        cache->remove = flexP_remove;
        cache->get_n_obj = flexP_get_n_obj;
        cache->get_occupied_byte = flexP_get_occupied_byte;
        cache->can_insert = flexP_can_insert;

        auto *params = reinterpret_cast<eviction::flexP_params_t *>(cache->eviction_params);

        params->req_local = new_request();
        params->req_prefetch = new_request();
        params->req_small = new_request();

        flexP_parse_params(cache, DEFAULT_CACHE_PARAMS);
        if (cache_specific_params != NULL)
        {
            flexP_parse_params(cache, cache_specific_params);
        }
        ...
        snprintf(cache->cache_name, CACHE_NAME_ARRAY_LEN, "flexP-%.2lf-%.2lf-%.2lf",
    ...
    static void flexP_free(cache_t *cache)
    {
        ...
        delete reinterpret_cast<eviction::flexP_params_t *>(cache->eviction_params);
    ...
    static cache_obj_t *flexP_prefetch(cache_t *cache, cache_obj_t *obj_toprefetch)
    {
        auto *params = reinterpret_cast<eviction::flexP_params_t *>(cache->eviction_params);
    ...
    static cache_obj_t *flexP_prefetchinsert(cache_t *cache)
    {
        auto *params = reinterpret_cast<eviction::flexP_params_t *>(cache->eviction_params);
            ...
            if (obj->FLEX.prefetch == 1 && obj->FLEX.prefetchhit == 0)
            {
                ...
                if (ghost_obj != NULL)
                {
                    ...
                    if (pobj != NULL && pobj->FLEX.prefetch == 0)
                    {
                        ...
                        flexP_prefetch(cache, pobj);
                        flexP_prefetchinsert(cache);
                    }
    ...
    static void flexP_adjustguard(cache_t *cache)
    {
        auto params = reinterpret_cast<eviction::flexP_params_t *>(cache->eviction_params);

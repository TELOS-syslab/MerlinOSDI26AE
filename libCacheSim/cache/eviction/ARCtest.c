...
typedef struct ARCtest_params {
  ...
  int objfromt1ghost;
  int hitfromt1ghost;
    int objhitfromt1ghost;
    int objfromt2ghost;
    int hitfromt2ghost;
    int objhitfromt2ghost;
} ARCtest_params_t;
...
static void ARCtest_parse_params(cache_t *cache, const char *cache_specific_params);
static void ARCtest_free(cache_t *cache);
static bool ARCtest_get(cache_t *cache, const request_t *req);
static cache_obj_t *ARCtest_find(cache_t *cache, const request_t *req,
                             const bool update_cache);
static cache_obj_t *ARCtest_insert(cache_t *cache, const request_t *req);
static cache_obj_t *ARCtest_to_evict(cache_t *cache, const request_t *req);
static void ARCtest_evict(cache_t *cache, const request_t *req);
static bool ARCtest_remove(cache_t *cache, const obj_id_t obj_id);

/* internal functions */
/* this is the case IV in the paper */
static void _ARCtest_evict_miss_on_all_queues(cache_t *cache, const request_t *req);
static void _ARCtest_replace(cache_t *cache, const request_t *req);
static cache_obj_t *_ARCtest_to_evict_miss_on_all_queues(cache_t *cache,
                                                     const request_t *req);
static cache_obj_t *_ARCtest_to_replace(cache_t *cache, const request_t *req);

/* debug functions */
static void print_cache(cache_t *cache);
static void _ARCtest_sanity_check(cache_t *cache, const request_t *req);
static inline void _ARCtest_sanity_check_full(cache_t *cache, const request_t *req);
static bool ARCtest_get_debug(cache_t *cache, const request_t *req);
...
cache_t *ARCtest_init(const common_cache_params_t ccache_params,
  ...
  cache->cache_init = ARCtest_init;
  cache->cache_free = ARCtest_free;
  cache->get = ARCtest_get;
  cache->find = ARCtest_find;
  cache->insert = ARCtest_insert;
  cache->evict = ARCtest_evict;
  cache->remove = ARCtest_remove;
  cache->to_evict = ARCtest_to_evict;
  ...
  cache->eviction_params = my_malloc_n(ARCtest_params_t, 1);
  ARCtest_params_t *params = (ARCtest_params_t *)(cache->eviction_params);
  params->p = 0;
  params->track_p = 0;
...
#ifdef USE_BELADY
  snprintf(cache->cache_name, CACHE_NAME_ARRAY_LEN, "ARCtest_Belady");
#endif
params->objfromt1ghost = 0;
params->hitfromt1ghost = 0;
    params->objhitfromt1ghost = 0;
    params->objfromt2ghost = 0;
    params->hitfromt2ghost = 0;
    params->objhitfromt2ghost = 0;
  return cache;
}
 ...
 */
static void ARCtest_free(cache_t *cache) {
  ARCtest_params_t *ARCtest_params = (ARCtest_params_t *)(cache->eviction_params);
  free_request(ARCtest_params->req_local);
  my_free(sizeof(ARCtest_params_t), ARCtest_params);
...
static bool ARCtest_get(cache_t *cache, const request_t *req) {
    ...
    if(abs(params->track_p - params->p) > 0.02 * cache->n_obj || (cache->n_req%1000000)==0){
        ...
        printf("reqnum: %d objfromt1ghost: %d hitfromt1ghost: %d objhitfromt1ghost: %d objfromt2ghost: %d hitfromt2ghost: %d objhitfromt2ghost: %d\n",
              cache->n_req,
           params->objfromt1ghost, params->hitfromt1ghost, params->objhitfromt1ghost,
           params->objfromt2ghost, params->hitfromt2ghost, params->objhitfromt2ghost);
    }
    #endif
#ifdef DEBUG_MODE
  return ARCtest_get_debug(cache, req);
#else
...
static cache_obj_t *ARCtest_find(cache_t *cache, const request_t *req,
  ...
  if (obj->ARC.ghost) {
    ...
    if(obj->ARC.fromt1){
        params->hitfromt1ghost +=1;
        if(obj->ARC.accessed == 0){
            params->objhitfromt1ghost +=1;
            obj->ARC.accessed = 1;
        }
    }
    if(obj->ARC.fromt2){
        params->hitfromt2ghost +=1;
        if(obj->ARC.accessed == 0){
            params->objhitfromt2ghost +=1;
            obj->ARC.accessed = 1;
        }
    }
  }
...
static cache_obj_t *ARCtest_insert(cache_t *cache, const request_t *req) {
  ...
  if (params->vtime_last_req_in_ghost == cache->n_req &&
    ...
    if(params->curr_obj_in_L1_ghost){
        params->objfromt1ghost +=1;
        obj->ARC.fromt1 = 1;
        obj->ARC.fromt2 = 0;
        obj->ARC.accessed = 0;
    }
    if(params->curr_obj_in_L2_ghost){
        params->objfromt2ghost +=1;
        obj->ARC.fromt2 = 1;
        obj->ARC.fromt1 = 0;
        obj->ARC.accessed = 0;
    }
...
static cache_obj_t *ARCtest_to_evict(cache_t *cache, const request_t *req) {
  ...
  if (params->vtime_last_req_in_ghost == cache->n_req &&
      (params->curr_obj_in_L1_ghost || params->curr_obj_in_L2_ghost)) {
    cache->to_evict_candidate = _ARCtest_to_replace(cache, req);
  } else {
    cache->to_evict_candidate = _ARCtest_to_evict_miss_on_all_queues(cache, req);
  }
...
static void ARCtest_evict(cache_t *cache, const request_t *req) {
  ...
  if (params->vtime_last_req_in_ghost == cache->n_req &&
      (params->curr_obj_in_L1_ghost || params->curr_obj_in_L2_ghost)) {
    _ARCtest_replace(cache, req);
  } else {
    _ARCtest_evict_miss_on_all_queues(cache, req);
  }
...
// ****                                                               ****
// ***********************************************************************
/* finding the eviction candidate in _ARCtest_replace but do not perform eviction */
static cache_obj_t *_ARCtest_to_replace(cache_t *cache, const request_t *req) {
  ARCtest_params_t *params = (ARCtest_params_t *)(cache->eviction_params);
...
static void _ARCtest_evict_L1_data(cache_t *cache, const request_t *req) {
  ARCtest_params_t *params = (ARCtest_params_t *)(cache->eviction_params);
...
static void _ARCtest_evict_L1_data_no_ghost(cache_t *cache, const request_t *req) {
  ARCtest_params_t *params = (ARCtest_params_t *)(cache->eviction_params);
  cache_obj_t *obj = params->L1_data_tail;
  DEBUG_ASSERT(obj != NULL);
...
static void _ARCtest_evict_L2_data(cache_t *cache, const request_t *req) {
  ARCtest_params_t *params = (ARCtest_params_t *)(cache->eviction_params);
...
static void _ARCtest_evict_L1_ghost(cache_t *cache, const request_t *req) {
  ARCtest_params_t *params = (ARCtest_params_t *)(cache->eviction_params);
...
static void _ARCtest_evict_L2_ghost(cache_t *cache, const request_t *req) {
  ARCtest_params_t *params = (ARCtest_params_t *)(cache->eviction_params);
...
static void _ARCtest_replace(cache_t *cache, const request_t *req) {
  ...
  if ((cond1 && (cond2 || cond3)) || cond4) {
    // delete the LRU in L1 data, move to L1_ghost
    _ARCtest_evict_L1_data(cache, req);
  } else {
    // delete the item in L2 data, move to L2_ghost
    _ARCtest_evict_L2_data(cache, req);
  }
}

/* finding the eviction candidate in _ARCtest_evict_miss_on_all_queues, but do not
 * perform eviction */
static cache_obj_t *_ARCtest_to_evict_miss_on_all_queues(cache_t *cache,
                                                     const request_t *req) {
  ARCtest_params_t *params = (ARCtest_params_t *)(cache->eviction_params);
  ...
  if (params->L1_data_size + params->L1_ghost_size + incoming_size >
    ...
    if (params->L1_ghost_size > 0) {
      return _ARCtest_to_replace(cache, req);
    } else {
...
/* this is the case IV in the paper */
static void _ARCtest_evict_miss_on_all_queues(cache_t *cache,
                                          const request_t *req) {
  ARCtest_params_t *params = (ARCtest_params_t *)(cache->eviction_params);
  ...
  if (params->L1_data_size + params->L1_ghost_size + incoming_size >
    ...
    if (params->L1_ghost_size > 0) {
      ...
      _ARCtest_evict_L1_ghost(cache, req);
      return _ARCtest_replace(cache, req);
    } else {
      // T1 >= c, L1 data size is too large, ghost is empty, so evict from L1
      // data
      return _ARCtest_evict_L1_data_no_ghost(cache, req);
    }
  } else {
    ...
    if (params->L1_data_size + params->L1_ghost_size + params->L2_data_size +
      ...
      if (params->L2_ghost_size > 0) {
        ...
        _ARCtest_evict_L2_ghost(cache, req);
      }
    }
    return _ARCtest_replace(cache, req);
  }
...
// ****                                                               ****
// ***********************************************************************
static const char *ARCtest_current_params(ARCtest_params_t *params) {
  static __thread char params_str[128];
  snprintf(params_str, 128, "\n");
  return params_str;
}

static void ARCtest_parse_params(cache_t *cache,
                             const char *cache_specific_params) {
  ARCtest_params_t *params = (ARCtest_params_t *)(cache->eviction_params);
...
static inline void _ARCtest_sanity_check_full(cache_t *cache,
                                          const request_t *req) {
  // if (cache->n_req < 13200000) return;

  _ARCtest_sanity_check(cache, req);
...
static bool ARCtest_get_debug(cache_t *cache, const request_t *req) {
  ...
  _ARCtest_sanity_check_full(cache, req);

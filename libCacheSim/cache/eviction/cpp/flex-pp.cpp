...
extern "C"
{
    ...
    static void flexpp_free(cache_t *cache);
    static bool flexpp_get(cache_t *cache, const request_t *req);

    static cache_obj_t *flexpp_find(cache_t *cache, const request_t *req,
                                   const bool update_cache);
    static cache_obj_t *flexpp_insert(cache_t *cache, const request_t *req);
    static cache_obj_t *flexpp_to_evict(cache_t *cache, const request_t *req);
    static void flexpp_evict(cache_t *cache, const request_t *req);
    static bool flexpp_remove(cache_t *cache, const obj_id_t obj_id);
    static inline int64_t flexpp_get_occupied_byte(const cache_t *cache);
    static inline int64_t flexpp_get_n_obj(const cache_t *cache);
    static inline bool flexpp_can_insert(cache_t *cache, const request_t *req);
    static void flexpp_update(cache_t *cache);
    static void flexpp_print(cache_t *cache);
    static void flexpp_aging(cache_t *cache);

    cache_t *flexpp_init(const common_cache_params_t ccache_params,
                        const char *cache_specific_params)
    {
        cache_t *cache = cache_struct_init("flexpp", ccache_params, cache_specific_params);
        cache->eviction_params = reinterpret_cast<void *>(new eviction::flexpp_params_t);

        cache->cache_init = flexpp_init;
        cache->cache_free = flexpp_free;
        cache->get = flexpp_get;
        cache->find = flexpp_find;
        cache->insert = flexpp_insert;
        cache->evict = flexpp_evict;
        cache->to_evict = flexpp_to_evict;
        cache->remove = flexpp_remove;
        cache->get_n_obj = flexpp_get_n_obj;
        cache->get_occupied_byte = flexpp_get_occupied_byte;
        cache->can_insert = flexpp_can_insert;

        auto *params = reinterpret_cast<eviction::flexpp_params_t *>(cache->eviction_params);
    ...
    static void flexpp_free(cache_t *cache)
    {
        ...
        delete reinterpret_cast<eviction::flexpp_params_t *>(cache->eviction_params);
...
static void printstatus(cache_t *cache);
    ...
    static cache_obj_t *flexpp_prefetch(cache_t *cache, cache_obj_t *obj_toprefetch)
    {
        auto *params = reinterpret_cast<eviction::flexpp_params_t *>(cache->eviction_params);
    ...
    static cache_obj_t *flexpp_prefetchinsert(cache_t *cache){
        auto *params = reinterpret_cast<eviction::flexpp_params_t *>(cache->eviction_params);
            ...
            if (obj->FLEX.prefetch==1&&obj->FLEX.prefetchhit==0)
            {
                ...
                if (ghost_obj != NULL)
                {
                    ...
                    if (pobj != NULL && pobj->FLEX.prefetch == 0)
                    {
                        ...
                        flexpp_prefetch(cache, pobj);
                        flexpp_prefetchinsert(cache);
                    }
                ...
                //set prefetch as not find
                params->prefetch_hittag = 1;
                return NULL;
            }
            else
            {
    ...
    {
        auto *params = reinterpret_cast<eviction::flexpp_params_t *>(cache->eviction_params);
        if(params->prefetch_hittag==1){
            cache_obj_t *obj = params->front->find(params->front, req, 0);
            if(obj != NULL){
                params->prefetch_hittag = 0;
                return NULL;
            }
    ...
    static void flexpp_adjustguard(cache_t *cache)
    {
        auto params = reinterpret_cast<eviction::flexpp_params_t *>(cache->eviction_params);

...
typedef struct Cacheusfix_params {
  cache_t *LRU;        // LRU
...
static void Cacheusfix_free(cache_t *cache);
static bool Cacheusfix_get(cache_t *cache, const request_t *req);
static cache_obj_t *Cacheusfix_find(cache_t *cache, const request_t *req,
                                 const bool update_cache);
static cache_obj_t *Cacheusfix_insert(cache_t *cache, const request_t *req);
static cache_obj_t *Cacheusfix_to_evict(cache_t *cache, const request_t *req);
static void Cacheusfix_evict(cache_t *cache, const request_t *req);
static bool Cacheusfix_remove(cache_t *cache, const obj_id_t obj_id);

static inline int64_t Cacheusfix_get_occupied_byte(const cache_t *cache);
static inline int64_t Cacheusfix_get_n_obj(const cache_t *cache);
static void Cacheusfix_parse_params(cache_t *cache,
...
/**
 * @brief initialize a Cacheusfix cache
 *
 * @param ccache_params some common cache parameters
 * @param cache_specific_params Cacheusfix specific parameters, should be NULL
 */
cache_t *Cacheusfix_init(const common_cache_params_t ccache_params,
  ...
  cache_t *cache = cache_struct_init("Cacheusfix", updated_cc_params, cache_specific_params);
  cache->cache_init = Cacheusfix_init;
  cache->cache_free = Cacheusfix_free;
  cache->get = Cacheusfix_get;
  cache->find = Cacheusfix_find;
  cache->insert = Cacheusfix_insert;
  cache->evict = Cacheusfix_evict;
  cache->remove = Cacheusfix_remove;
  cache->to_evict = Cacheusfix_to_evict;
  cache->can_insert = cache_can_insert_default;
  cache->get_n_obj = Cacheusfix_get_n_obj;
  cache->get_occupied_byte = Cacheusfix_get_occupied_byte;
  ...
  cache->eviction_params = my_malloc_n(Cacheusfix_params_t, 1);
  Cacheusfix_params_t *params = (Cacheusfix_params_t *)(cache->eviction_params);
  ...
  Cacheusfix_parse_params(cache, DEFAULT_CACHE_PARAMS);
  if (cache_specific_params != NULL) {
    Cacheusfix_parse_params(cache, cache_specific_params);
  }
...
static void Cacheusfix_free(cache_t *cache) {
  ...
  my_free(sizeof(Cacheusfix_params_t), params);
...
static bool Cacheusfix_get(cache_t *cache, const request_t *req) {
  ...
  if (abs(params->track_wlru - params->w_lru) > 0.02 || (cache->n_req%1000000)==0) {
    ...
    printf("%ld Cacheusfix w_lru: %.4lf w_lfu: %.4lf learning_rate: %.4lf\n", cache->n_req, params->w_lru, params->w_lfu, params->lr);
  }
  #endif
...
static void Cacheusfix_evict(cache_t *cache, const request_t *req) {
  ...
  if (cache->to_evict_candidate_gen_vtime == cache->n_req) {
    ...
    obj_to_evict = Cacheusfix_to_evict(cache, req);
  }

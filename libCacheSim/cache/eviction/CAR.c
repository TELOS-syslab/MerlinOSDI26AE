...
static void _CAR_evict_miss_on_all_queues(cache_t *cache, const request_t *req);
...
static inline void _CAR_sanity_check_full(cache_t *cache, const request_t *req);
static bool CAR_get_debug(cache_t *cache, const request_t *req);
 ...
 */
static void CAR_free(cache_t *cache) {
  CAR_params_t *CAR_params = (CAR_params_t *)(cache->eviction_params);
  free_request(CAR_params->req_local);
  my_free(sizeof(CAR_params_t), CAR_params);
...
static void CAR_cycle_L1(cache_t *cache, const request_t *req) {
    CAR_params_t *params = (CAR_params_t *)(cache->eviction_params);
    while(params->L1_data_tail != NULL && params->L1_data_tail->CAR.access){
...
static void CAR_cycle_L2(cache_t *cache, const request_t *req) {
    CAR_params_t *params = (CAR_params_t *)(cache->eviction_params);
    while(params->L2_data_tail != NULL && params->L2_data_tail->CAR.access){
...
static void CAR_evict(cache_t *cache, const request_t *req) {
  ...
  if (params->vtime_last_req_in_ghost == cache->n_req &&
    ...
    _CAR_evict_miss_on_all_queues(cache, req);
  }
...
// ****                                                               ****
// ***********************************************************************
/* finding the eviction candidate in _CAR_replace but do not perform eviction */
static cache_obj_t *_CAR_to_replace(cache_t *cache, const request_t *req) {
  CAR_params_t *params = (CAR_params_t *)(cache->eviction_params);
  ...
  if ((cond1 && (cond2 || cond3)) || cond4) {
    // check access bit in L1
    CAR_cycle_L1(cache, req);
    if(params->L1_data_tail == NULL){
        CAR_cycle_L2(cache, req);
        obj = params->L2_data_tail;
    }else{
        obj = params->L1_data_tail;
    }
  } else {
    // check access bit in L2
    CAR_cycle_L2(cache, req);
    obj = params->L2_data_tail;
  }
...
static bool _CAR_evict_L1_data(cache_t *cache, const request_t *req) {
  CAR_params_t *params = (CAR_params_t *)(cache->eviction_params);
  CAR_cycle_L1(cache, req);
  cache_obj_t *obj = params->L1_data_tail;
    if(obj==NULL){
...
static void _CAR_evict_L1_data_no_ghost(cache_t *cache, const request_t *req) {
  CAR_params_t *params = (CAR_params_t *)(cache->eviction_params);
    CAR_cycle_L1(cache, req);
  cache_obj_t *obj = params->L1_data_tail;
  if(obj==NULL){
...
static void _CAR_evict_L2_data(cache_t *cache, const request_t *req) {
  CAR_params_t *params = (CAR_params_t *)(cache->eviction_params);
  CAR_cycle_L2(cache, req);
  cache_obj_t *obj = params->L2_data_tail;
  DEBUG_ASSERT(obj != NULL);
...
static void _CAR_evict_L1_ghost(cache_t *cache, const request_t *req) {
  CAR_params_t *params = (CAR_params_t *)(cache->eviction_params);
...
static void _CAR_evict_L2_ghost(cache_t *cache, const request_t *req) {
  CAR_params_t *params = (CAR_params_t *)(cache->eviction_params);
...
static void _CAR_replace(cache_t *cache, const request_t *req) {
  ...
  if ((cond1 && (cond2 || cond3)) || cond4) {
    ...
    bool ret = _CAR_evict_L1_data(cache, req);
    if(ret == false){
        //L1 is empty, evict from L2
        _CAR_evict_L2_data(cache, req);
    }
  } else {
...
/* this is the case IV in the paper */
static void _CAR_evict_miss_on_all_queues(cache_t *cache,
                                          const request_t *req) {
  CAR_params_t *params = (CAR_params_t *)(cache->eviction_params);
  ...
  if (params->L1_data_size + params->L1_ghost_size + incoming_size >
    ...
    if (params->L1_ghost_size > 0) {
      ...
      _CAR_evict_L1_ghost(cache, req);
      return _CAR_replace(cache, req);
    } else {
      // T1 >= c, L1 data size is too large, ghost is empty, so evict from L1
      // data
      return _CAR_evict_L1_data_no_ghost(cache, req);
    }
  } else {
...
// ****                                                               ****
// ***********************************************************************
static const char *CAR_current_params(CAR_params_t *params) {

...
#define FreqMask 0x7
        ...
        FOLLY_ALWAYS_INLINE int getFreq(const T &node) const noexcept
        {
            ...
            int ret = __atomic_load_n(&node.ref_.refCount_, __ATOMIC_RELAXED);
            return (ret >> RefFlags::kMMFlag2) & (FLEX_MAX_FREQ);
        }
        ...
        void setFreq(T &node, int freq) noexcept
        {
            ...
            resetFreq(node);
            Value bitMask = ((static_cast<Value>(freq) & FLEX_MAX_FREQ) << RefFlags::kMMFlag2);
            __atomic_or_fetch(&node.ref_.refCount_, bitMask, __ATOMIC_ACQ_REL);
            return;
        }
    ...
    private:
        ...
        std::vector<std::atomic<uint32_t>> freq_distribution_{std::vector<std::atomic<uint32_t>>(FLEX_MAX_FREQ + 1)};
